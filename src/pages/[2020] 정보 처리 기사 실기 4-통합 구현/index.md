...
- `직접 연결` : _DB Connection Pool, DB Link, JDBC, API 등(DDJA)_

- `간접 연결` : _EAI, Web Service/ESB, Socket 등(EWS)_
...
- `EAI(Enterprise Application Integration)`(PHMB) : 기업내 각종 애플리케이션 및 플랫폼 간의 정보 전달, 연계, 통합 등 상호 연동이 가능하게 해주는 솔루션
...
- `웹 서비스(Web Service)`(SWU) : 네트워크의 정보를 표준화된 서비스 형태로 만들어 공유하는 기술로, 서비스 지향 아키텍처(SOA) 개념을 실현하는 대표적인 기술
