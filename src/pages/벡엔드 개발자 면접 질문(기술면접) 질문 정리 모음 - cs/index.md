...
* 클라이언트가 서버에서 HTTP Request를 주기적으로 요청하고, 서버가 응답하는 방식이다. 
...
* _실행 중인 프로그램_, 운영체제로부터 자원을 할당, 한 프로세스 안에 여러개의 스레드가 생성될 수 있음
...
* 대상의 작업이 끝날 때 까지 제어권을 대상이 가지고 있는 것을 의미합니다. 반면에 논블로킹은 대상의 작업 완료여부와 상관없이 새로운 작업을 수행
...
* 상호배제 (Mutual exclusion) : 한번에 한 프로세스만이 자원을 점유할 수 있음
* 점유대기 (Hold and wait): 프로세스가 이미 자원을 점유하는 상태에서 다른 자원을 무한정 기다림
* 비선점 (No preemption): 프로세스가 어떤 자원의 점유를 끝낼 때까지 그 자원을 뺏을 수 없음
* 환형대기 (Circular wait): 각 프로세스들이 원형으로 구성되어 순환적으로 자원을 요구
