...
* 기존에 무시되던 에러들이 throw 되며 JS 엔진 최적화 작업을 어렵게 만드는 실수들을 바로 잡고 ECMAScript의 차기 버전에서 정의될 문법들을 금지하는 특성을 가지고 있음
...
_자바스크립트의 코드가 실행되기 위해 정보들을 담고 있는 곳_(변수 객체, 스코프 체인, this 정보), 자바스크립트의 코드들이 실행되기 위한 환경(전역 컨텍스트 ,함수 컨텍스트)  
