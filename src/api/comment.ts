...
// 댓글쓰기
export const writecommentApi  = async ( writecomment: IWriteCommentForm,  thunkAPI: any) => {
...
export const removeCommentApi = async ( removecomment: IRemoveCommentForm,  thunkAPI: any) => {
    const res : any = await fetch(`${SERVER_URL}/comments${removecomment.postId}${removecomment.commentId}`,{
        method: "DELETE",
        headers: {
            "Content-Type": "application/json",
        },
