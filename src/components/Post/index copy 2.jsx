...
const PostTemplate = ({
  ...
  useEffect(()=>{
    ...
    tocItems.forEach(tocitem => {
      ...
      tocitem.classList.add(`toctextlink${toctext}`)
    })
    setTocEls(tocItems)
    const observer = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        if (entry.isIntersecting){
          ...
          const tocItem = document.querySelector(`#${entry.target.innerText.replace(/[0-9|\.|\s]/g, '')}`)
          const toctextitem =  document.querySelector(`.toctextlink${entry.target.innerText.replace(/[0-9|\.|\s]/g, '')}`)
  ...
  // 스크롤하기
  const onClick = (e) => {
    const targettext = document.querySelector(`.toctextlink${e.currentTarget.id}`)
    window.scrollTo({
          ...
          <Grid item lg={2} md={2} sm={false} xs={false}>
            <VisibleTable>
              ...
              <TocItemDiv 
                ...
                id={`${item.innerText.replace(/[0-9|\.|\s]/g, '')}`}
                onClick={e => onClick(e)}>
