...
const SmallPagination = ({ postCount, location, prefix }) => {
  const pageCount = postCount
    ? Math.ceil(postCount / CONTENT_PER_SMALL_PAGE)
    : 0;
  const pages = Array.from(new Array(pageCount), (cnt, i) => i + 1);
  const page = getPage(location);
  const hasManyPages = pageCount >= PAGE_PER_SMALL_SCREEN;
  const filteredPages = hasManyPages
    ? pages.filter(
        (p) => Math.abs(page - p) <= Math.floor(PAGE_PER_SMALL_SCREEN / 2)
      )
    : pages;
